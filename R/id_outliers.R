...
#' @param iqr_or_sd a single string character of "iqr" or "sd" to determine if outlier should be determined by means and sd or medians and iqr.
#' @param number_of_sd_iqr a single numeric value to define a cutoff value for how many iqr or sd units outlier values.
#' @return returns vector indicating the index of an outlier(s).
#' @importFrom stats sd median quantile
#' @export
#' @examples
#' id_outliers()
id_outliers = function( y , iqr_or_sd = "iqr", number_of_sd_iqr = 5){
  ...
  sdy = sd(y, na.rm = TRUE)
  sd_cutoff = c( meany - (number_of_sd_iqr*sdy) , meany + (number_of_sd_iqr*sdy) )

  ## (2) using median and IQR
  mediany = median(y, na.rm = TRUE)
  iqry = quantile(y, probs = c(0.25, 0.75), na.rm = TRUE)
  iqry = iqry[2] - iqry[1]
  iqr_cutoff = c( mediany - (number_of_sd_iqr*iqry) , mediany + (number_of_sd_iqr*iqry) )
